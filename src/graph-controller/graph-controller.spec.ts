import { GraphStore } from "@/graph-store";
import { CoreHtmlView, HtmlView, LayoutHtmlView } from "@/html-view";
import { ViewportStore } from "@/viewport-store";
import { GraphController } from "./graph-controller";
import { createElement } from "@/mocks";
import { CenterFn, standardCenterFn } from "@/center-fn";
import { BezierEdgeShape } from "@/edges";
import { PriorityFn } from "@/priority";
import { GraphControllerParams } from "./graph-controller-params";
import { EdgeShapeFactory } from "./edge-shape-factory";

const createGraphController = (options?: {
  element?: HTMLElement;
  nodesCenterFn?: CenterFn;
  nodesPriorityFn?: PriorityFn;
  portsDirection?: number;
  edgeShapeFactory?: EdgeShapeFactory;
  edgesPriorityFn?: PriorityFn;
}): { graphController: GraphController; graphStore: GraphStore } => {
  const element = options?.element ?? document.createElement("div");
  const graphStore = new GraphStore();
  const viewportStore = new ViewportStore(element);
  let htmlView: HtmlView = new CoreHtmlView(graphStore, viewportStore, element);
  htmlView = new LayoutHtmlView(htmlView, graphStore);

  const params: GraphControllerParams = {
    nodes: {
      centerFn: options?.nodesCenterFn ?? standardCenterFn,
      priorityFn: options?.nodesPriorityFn ?? ((): number => 0),
    },
    ports: {
      direction: options?.portsDirection ?? 0,
    },
    edges: {
      shapeFactory:
        options?.edgeShapeFactory ??
        ((): BezierEdgeShape => new BezierEdgeShape()),
      priorityFn: options?.edgesPriorityFn ?? ((): number => 0),
    },
  };

  const graphController = new GraphController(graphStore, htmlView, params);

  return { graphController, graphStore };
};

describe("GraphController", () => {
  it("should add node", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });

    graphController.addNode({
      element: createElement(),
      x: 0,
      y: 0,
    });

    const container = element.children[0].children[0];

    expect(container.children.length).toBe(1);
  });

  it("should set node coordinates to null when not specified", () => {
    const element = document.createElement("div");
    const { graphController, graphStore } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
    });

    const node = graphStore.getNode("node-1");

    expect({ x: node.payload.x, y: node.payload.y }).toEqual({
      x: null,
      y: null,
    });
  });

  it("should add node with specified id", () => {
    const element = document.createElement("div");
    const { graphController, graphStore } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    const node = graphStore.getNode("node-1");

    expect(node).not.toBe(null);
  });

  it("should add node with specified default centerFn", () => {
    const element = document.createElement("div");
    const centerFn: CenterFn = () => ({ x: 0, y: 0 });

    const { graphController } = createGraphController({
      element,
      nodesCenterFn: centerFn,
    });

    graphController.addNode({
      id: "node-1",
      element: createElement({ width: 100, height: 100 }),
      x: 0,
      y: 0,
    });

    const container = element.children[0].children[0];
    const nodeElement = container.children[0] as HTMLElement;

    expect(nodeElement.style.transform).toBe("translate(0px, 0px)");
  });

  it("should add node with specified centerFn", () => {
    const element = document.createElement("div");
    const centerFn: CenterFn = () => ({ x: 0, y: 0 });

    const { graphController } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement({ width: 100, height: 100 }),
      x: 0,
      y: 0,
      centerFn,
    });

    const container = element.children[0].children[0];
    const nodeElement = container.children[0] as HTMLElement;

    expect(nodeElement.style.transform).toBe("translate(0px, 0px)");
  });

  it("should add node with specified default priority", () => {
    const element = document.createElement("div");

    const { graphController } = createGraphController({
      element,
      nodesPriorityFn: () => 10,
    });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    const container = element.children[0].children[0];
    const nodeElement = container.children[0] as HTMLElement;

    expect(nodeElement.style.zIndex).toBe("10");
  });

  it("should add node with specified priority", () => {
    const element = document.createElement("div");

    const { graphController } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
      priority: 10,
    });

    const container = element.children[0].children[0];
    const nodeElement = container.children[0] as HTMLElement;

    expect(nodeElement.style.zIndex).toBe("10");
  });

  it("should mark specified ports when adding node", () => {
    const element = document.createElement("div");

    const { graphController } = createGraphController({ element });

    const spy = jest.spyOn(graphController, "markPort");

    const portElement = createElement();

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
      ports: [
        {
          id: "port-1",
          element: portElement,
        },
      ],
    });

    expect(spy).toHaveBeenCalledWith({
      id: "port-1",
      nodeId: "node-1",
      element: portElement,
    });
  });

  it("should update node without arguments", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });
    const nodeElement = createElement();

    graphController.addNode({
      id: "node-1",
      element: nodeElement,
      x: 0,
      y: 0,
    });

    nodeElement.getBoundingClientRect = (): DOMRect => {
      return new DOMRect(0, 0, 100, 100);
    };

    graphController.updateNode("node-1");

    const container = element.children[0].children[0];
    const nodeElementWrapper = container.children[0] as HTMLElement;

    expect(nodeElementWrapper.style.transform).toBe("translate(-50px, -50px)");
  });

  it("should update node coordinates", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    graphController.updateNode("node-1", { x: 100, y: 100 });

    const container = element.children[0].children[0];
    const nodeElement = container.children[0] as HTMLElement;

    expect(nodeElement.style.transform).toBe("translate(100px, 100px)");
  });

  it("should update node centerFn", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement({ width: 100, height: 100 }),
      x: 0,
      y: 0,
    });

    graphController.updateNode("node-1", { centerFn: () => ({ x: 0, y: 0 }) });

    const container = element.children[0].children[0];
    const nodeElement = container.children[0] as HTMLElement;

    expect(nodeElement.style.transform).toBe("translate(0px, 0px)");
  });

  it("should update node priority", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    graphController.updateNode("node-1", { priority: 10 });

    const container = element.children[0].children[0];
    const nodeElement = container.children[0] as HTMLElement;

    expect(nodeElement.style.zIndex).toBe("10");
  });

  it("should remove node", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    graphController.removeNode("node-1");

    const container = element.children[0].children[0];

    expect(container.children.length).toBe(0);
  });

  it("should mark port", () => {
    const element = document.createElement("div");
    const { graphController, graphStore } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    graphController.markPort({
      nodeId: "node-1",
      element: createElement(),
    });

    expect(graphStore.getAllPortIds().length).toBe(1);
  });

  it("should mark port with specified id", () => {
    const element = document.createElement("div");
    const { graphController, graphStore } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    graphController.markPort({
      id: "port-1",
      nodeId: "node-1",
      element: createElement(),
    });

    expect(graphStore.hasPort("port-1")).toBe(true);
  });

  it("should mark port with specified default port direction", () => {
    const element = document.createElement("div");
    const { graphController, graphStore } = createGraphController({
      element,
      portsDirection: Math.PI,
    });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    graphController.markPort({
      id: "port-1",
      nodeId: "node-1",
      element: createElement(),
    });

    const port = graphStore.getPort("port-1");

    expect(port.payload.direction).toBe(Math.PI);
  });

  it("should mark port with specified port direction", () => {
    const element = document.createElement("div");
    const { graphController, graphStore } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    graphController.markPort({
      id: "port-1",
      nodeId: "node-1",
      element: createElement(),
      direction: Math.PI,
    });

    const port = graphStore.getPort("port-1");

    expect(port.payload.direction).toBe(Math.PI);
  });

  it("should update port direction", () => {
    const element = document.createElement("div");
    const { graphController, graphStore } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    graphController.markPort({
      id: "port-1",
      nodeId: "node-1",
      element: createElement(),
    });

    graphController.updatePort("port-1", { direction: Math.PI });

    const port = graphStore.getPort("port-1");

    expect(port.payload.direction).toBe(Math.PI);
  });

  it("should unmark port", () => {
    const element = document.createElement("div");
    const { graphController, graphStore } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    graphController.markPort({
      id: "port-1",
      nodeId: "node-1",
      element: createElement(),
    });

    graphController.unmarkPort("port-1");

    expect(graphStore.getAllPortIds().length).toBe(0);
  });

  it("should add edge", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    graphController.markPort({
      id: "port-1",
      nodeId: "node-1",
      element: createElement(),
    });

    graphController.addEdge({ from: "port-1", to: "port-1" });

    const container = element.children[0].children[0];

    expect(container.children.length).toBe(2);
  });

  it("should add edge with specified id", () => {
    const element = document.createElement("div");
    const { graphController, graphStore } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    graphController.markPort({
      id: "port-1",
      nodeId: "node-1",
      element: createElement(),
    });

    graphController.addEdge({ id: "edge-1", from: "port-1", to: "port-1" });

    expect(graphStore.getEdge("edge-1")).not.toBe(null);
  });

  it("should add edge with specified default shape", () => {
    const element = document.createElement("div");
    const shape = new BezierEdgeShape();
    const { graphController } = createGraphController({
      element,
      edgeShapeFactory: () => shape,
    });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    graphController.markPort({
      id: "port-1",
      nodeId: "node-1",
      element: createElement(),
    });

    graphController.addEdge({ id: "edge-1", from: "port-1", to: "port-1" });

    const container = element.children[0].children[0];
    const edgeSvg = container.children[1];

    expect(edgeSvg).toBe(shape.svg);
  });

  it("should add edge with specified shape", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    graphController.markPort({
      id: "port-1",
      nodeId: "node-1",
      element: createElement(),
    });

    const shape = new BezierEdgeShape();

    graphController.addEdge({
      id: "edge-1",
      from: "port-1",
      to: "port-1",
      shape,
    });

    const container = element.children[0].children[0];
    const edgeSvg = container.children[1];

    expect(edgeSvg).toBe(shape.svg);
  });

  it("should add edge with specified default priority", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({
      element,
      edgesPriorityFn: () => 10,
    });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    graphController.markPort({
      id: "port-1",
      nodeId: "node-1",
      element: createElement(),
    });

    graphController.addEdge({ from: "port-1", to: "port-1" });

    const container = element.children[0].children[0];
    const edgeSvg = container.children[1] as SVGSVGElement;

    expect(edgeSvg.style.zIndex).toBe("10");
  });

  it("should add edge with specified priority", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    graphController.markPort({
      id: "port-1",
      nodeId: "node-1",
      element: createElement(),
    });

    graphController.addEdge({
      id: "edge-1",
      from: "port-1",
      to: "port-1",
      priority: 10,
    });

    const container = element.children[0].children[0];
    const edgeSvg = container.children[1] as SVGSVGElement;

    expect(edgeSvg.style.zIndex).toBe("10");
  });

  it("should update edge without parameters", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    const portElement = createElement();

    graphController.markPort({
      id: "port-1",
      nodeId: "node-1",
      element: portElement,
    });

    graphController.addEdge({ id: "edge-1", from: "port-1", to: "port-1" });

    portElement.getBoundingClientRect = (): DOMRect => {
      return new DOMRect(100, 100, 0, 0);
    };

    graphController.updateEdge("edge-1");

    const container = element.children[0].children[0];
    const edgeSvg = container.children[1] as SVGSVGElement;

    expect(edgeSvg.style.transform).toBe("translate(100px, 100px)");
  });

  it("should update edge source", () => {
    const element = document.createElement("div");
    const { graphController, graphStore } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
      ports: [
        {
          id: "port-1",
          element: createElement(),
        },
      ],
    });

    graphController.addNode({
      id: "node-2",
      element: createElement(),
      x: 0,
      y: 0,
      ports: [
        {
          id: "port-2",
          element: createElement(),
        },
      ],
    });

    graphController.addEdge({ id: "edge-1", from: "port-1", to: "port-2" });

    graphController.updateEdge("edge-1", { from: "port-2" });

    const edge = graphStore.getEdge("edge-1")!;

    expect(edge.from).toBe("port-2");
  });

  it("should update edge target", () => {
    const element = document.createElement("div");
    const { graphController, graphStore } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
      ports: [
        {
          id: "port-1",
          element: createElement(),
        },
      ],
    });

    graphController.addNode({
      id: "node-2",
      element: createElement(),
      x: 0,
      y: 0,
      ports: [
        {
          id: "port-2",
          element: createElement(),
        },
      ],
    });

    graphController.addEdge({ id: "edge-1", from: "port-1", to: "port-2" });

    graphController.updateEdge("edge-1", { to: "port-1" });

    const edge = graphStore.getEdge("edge-1")!;

    expect(edge.to).toBe("port-1");
  });

  it("should update edge priority", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
      ports: [
        {
          id: "port-1",
          element: createElement(),
        },
      ],
    });

    graphController.addEdge({ id: "edge-1", from: "port-1", to: "port-1" });

    graphController.updateEdge("edge-1", { priority: 10 });

    const container = element.children[0].children[0];
    const edgeSvg = container.children[1] as SVGSVGElement;

    expect(edgeSvg.style.zIndex).toBe("10");
  });

  it("should update edge shape", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
      ports: [
        {
          id: "port-1",
          element: createElement(),
        },
      ],
    });

    const newShape = new BezierEdgeShape();

    graphController.addEdge({ id: "edge-1", from: "port-1", to: "port-1" });

    graphController.updateEdge("edge-1", { shape: newShape });

    const container = element.children[0].children[0];
    const edgeSvg = container.children[1] as SVGSVGElement;

    expect(edgeSvg).toBe(newShape.svg);
  });

  it("should remove edge", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
      ports: [
        {
          id: "port-1",
          element: createElement(),
        },
      ],
    });

    graphController.addEdge({ id: "edge-1", from: "port-1", to: "port-1" });
    graphController.removeEdge("edge-1");

    const container = element.children[0].children[0];

    expect(container.children.length).toBe(1);
  });

  it("should clear graphController from elements", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });

    graphController.addNode({
      id: "node-1",
      element: createElement(),
      x: 0,
      y: 0,
    });

    graphController.clear();

    const container = element.children[0].children[0];

    expect(container.children.length).toBe(0);
  });

  it("should reset node id generator", () => {
    const element = document.createElement("div");
    const { graphController, graphStore } = createGraphController({ element });

    graphController.addNode({
      element: createElement(),
      x: 0,
      y: 0,
    });
    graphController.addNode({
      element: createElement(),
      x: 0,
      y: 0,
    });

    graphController.clear();

    graphController.addNode({
      element: createElement(),
      x: 0,
      y: 0,
    });

    expect(graphStore.getAllNodeIds()).toEqual([0]);
  });

  it("should reset port id generator", () => {
    const element = document.createElement("div");
    const { graphController, graphStore } = createGraphController({ element });

    const addNode = (): void => {
      graphController.addNode({
        element: createElement(),
        x: 0,
        y: 0,
        ports: [
          {
            element: createElement(),
          },
        ],
      });
    };

    addNode();
    addNode();
    graphController.clear();
    addNode();

    expect(graphStore.getAllPortIds()).toEqual([0]);
  });

  it("should reset edge id generator", () => {
    const element = document.createElement("div");
    const { graphController, graphStore } = createGraphController({ element });

    const addElements = (): void => {
      graphController.addNode({
        element: createElement(),
        x: 0,
        y: 0,
        ports: [
          {
            id: "port-1",
            element: createElement(),
          },
        ],
      });

      graphController.addEdge({ from: "port-1", to: "port-1" });
    };

    addElements();
    graphController.addEdge({ from: "port-1", to: "port-1" });
    graphController.clear();
    addElements();

    expect(graphStore.getAllEdgeIds()).toEqual([0]);
  });

  it("should clear before destroy", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });

    const spy = jest.spyOn(graphController, "clear");

    graphController.destroy();

    expect(spy).toHaveBeenCalled();
  });

  it("should clear html on destroy", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });

    graphController.destroy();

    expect(element.children.length).toBe(0);
  });

  it("should render adjacent edges when updating node", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });
    const nodeElement = createElement();

    graphController.addNode({
      id: "node-1",
      element: nodeElement,
      x: 0,
      y: 0,
      ports: [
        {
          id: "port-1",
          element: createElement(),
        },
      ],
    });

    const shape = new BezierEdgeShape();
    const spy = jest.spyOn(shape, "render");

    graphController.addEdge({ from: "port-1", to: "port-1", shape });
    graphController.updateNode("node-1");

    expect(spy).toHaveBeenCalled();
  });

  it("should unmark ports before node removal", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });
    const nodeElement = createElement();

    graphController.addNode({
      id: "node-1",
      element: nodeElement,
      x: 0,
      y: 0,
      ports: [
        {
          id: "port-1",
          element: createElement(),
        },
      ],
    });

    const spy = jest.spyOn(graphController, "unmarkPort");

    graphController.removeNode("node-1");

    expect(spy).toHaveBeenCalledWith("port-1");
  });

  it("should render adjacent edges when updating port", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });
    const nodeElement = createElement();

    graphController.addNode({
      id: "node-1",
      element: nodeElement,
      x: 0,
      y: 0,
      ports: [
        {
          id: "port-1",
          element: createElement(),
        },
      ],
    });

    const shape = new BezierEdgeShape();
    const spy = jest.spyOn(shape, "render");

    graphController.addEdge({ from: "port-1", to: "port-1", shape });
    graphController.updatePort("port-1");

    expect(spy).toHaveBeenCalled();
  });

  it("should remove adjacent edges before port unmark", () => {
    const element = document.createElement("div");
    const { graphController } = createGraphController({ element });
    const nodeElement = createElement();

    graphController.addNode({
      id: "node-1",
      element: nodeElement,
      x: 0,
      y: 0,
      ports: [
        {
          id: "port-1",
          element: createElement(),
        },
      ],
    });

    graphController.addEdge({ id: "edge-1", from: "port-1", to: "port-1" });

    const spy = jest.spyOn(graphController, "removeEdge");

    graphController.unmarkPort("port-1");

    expect(spy).toHaveBeenCalledWith("edge-1");
  });
});
