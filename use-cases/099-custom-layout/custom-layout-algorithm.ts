...
interface CustomLayoutAlgorithmParams {
...
export class CustomLayoutAlgorithm implements LayoutAlgorithm {
  public constructor(private readonly params: CustomLayoutAlgorithmParams) {}
  ...
  private calculateLayers(
    graph: Graph,
  ): ReadonlyMap<number, ReadonlySet<Identifier>> {
    ...
    while (currentLayerStack.length !== 0) {
      ...
      currentLayerStack.forEach((nodeId) => {
        const outgoingEdges = graph.getNodeOutgoingEdgeIds(nodeId);
